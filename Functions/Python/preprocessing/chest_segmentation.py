...
def lung_fill_hole_dirlab(cn, data):
    data_folder = 'E:/PHD/Database/'
    if data == 'DIR-Lab_COPD':
        type_im_max = 2
    elif data == 'DIR-Lab_4DCT':
        type_im_max = 10
    else:
        raise ValueError('data should be in ["DIR-Lab_4DCT", "DIR-Lab_COPD"]')
    for type_im in range(0, type_im_max):
        lung_fill_hole(data_folder=data_folder, data=data, type_im=type_im, cn=cn)


def ptpulmo_segmentation_dirlab(cn, data, segment_organ):
    data_folder = 'E:/PHD/Database/'
    if data == 'DIR-Lab_COPD':
        type_im_max = 2
    elif data == 'DIR-Lab_4DCT':
        ...
        raise ValueError('data should be in ["DIR-Lab_4DCT", "DIR-Lab_COPD"]')
    for type_im in range(0, type_im_max):
        ptpulmo_segmentation(data_folder=data_folder, data=data, type_im=type_im, cn=cn, segment_organ=segment_organ)
...
if __name__ == '__main__':
    ...
    data = 'DIR-Lab_COPD'
    segment_organ = 'Torso'
    ptpulmo_segmentation_dirlab(cn=cn, data=data, segment_organ=segment_organ)